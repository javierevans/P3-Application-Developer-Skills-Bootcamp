...
class Tournament:
    ...
    def __init__(
        ...
        # Store the tournament players
        self.players = players if players is not None else []

        # Track each player's score
        self.player_scores = {}

        # Store every completed round
        self.rounds = rounds if rounds is not None else []

    def __str__(self):
        """Return a readable tournament summary."""

        return (
            f"Tournament: {self.name} | "
            f"Location: {self.location} | "
            f"Players: {len(self.players)} | "
            f"Round: {self.current_round}/{self.number_of_rounds}"
        )
    ...
    def serialize(self):
        """Convert the tournament into a dictionary."""
    ...
    def can_start(self):
        """Return True if the tournament is ready to start."""
    ...
    def generate_first_round_matches(self, round_obj):
        ...
        for index in range(len(top_half)):
            player_one = top_half[index]
            player_two = bottom_half[index]
    ...
    def sort_players_by_score(self):
        ...
        return sorted(
            self.players,
            key=lambda player: (
                self.player_scores[player.chess_id],
                player.chess_id,
            ),
            reverse=True,
        )
    ...
    def pair_score_groups(self, score_groups, round_obj):
        """Pair players inside each score group."""
        ...
        for score in score_groups:
            ...
            while remaining_players:
                ...
                for index in range(1, len(remaining_players)):
                    player_two = remaining_players[index]

                    if not self.have_played_before(
                        player_one,
                        player_two,
                    ):
                        match = Match(
    ...
    def have_played_before(self, player_one, player_two):
        """Return True if the players have already played."""
        for round_obj in self.rounds:
            for match in round_obj.matches:
                if (
                    player_one in match.players
                    and player_two in match.players
                ):
                    return True
    ...
    def record_match_result(self, match, winner=None):
        ...
        # Save the tournament after every result
        self.save(
            f"tournament_round_{self.current_round}.json"
        )

        # Move to the next round if every match is complete
    ...
    def from_dict(cls, data):
        ...
        # Rebuild the player scores from the saved matches
